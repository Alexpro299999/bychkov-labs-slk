    ...
    public partial class MainWindow : Window
    {
        ...
        private void LoadData()
        {
            try
            {
                ...
                ManufacturersGrid.ItemsSource = _manufacturerRepo.GetAll();
                WatchesGrid.ItemsSource = _watchRepo.GetAll();
                StockGrid.ItemsSource = _stockRepo.GetAll(); 
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Критическая ошибка загрузки данных: {ex.Message}");
            }
        }
        ...
        private void AddManufacturer_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (addWindow.ShowDialog() == true)
            {
                try
                {
                    _manufacturerRepo.Add(addWindow.CurrentManufacturer);
                    LoadData();
                }
        ...
        private void EditManufacturer_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (editWindow.ShowDialog() == true)
            {
                try
                {
                    _manufacturerRepo.Update(editWindow.CurrentManufacturer);
                    LoadData();
                }
        ...
        private void DeleteManufacturer_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                int watchCount = _watchRepo.CountWatchesByManufacturer(selected.ID);
                if (watchCount > 0)
                {
        ...
        private void AddWatch_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (addWindow.ShowDialog() == true)
            {
                try
                {
                    _watchRepo.Add(addWindow.CurrentWatch);
                    LoadData();
                }
                catch (Exception ex) { MessageBox.Show($"Ошибка добавления: {ex.Message}"); }
            }
        ...
        private void EditWatch_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (editWindow.ShowDialog() == true)
            {
                try
                {
                    _watchRepo.Update(editWindow.CurrentWatch);
                    LoadData();
                }
                catch (Exception ex) { MessageBox.Show($"Ошибка изменения: {ex.Message}"); }
            }
        ...
        private void QueryWatchesByType_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                var result = _watchRepo.GetWatchModelsByType(dialog.ResponseText);
        ...
        private void QueryMechanicalCheaperThan_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (dialog.ShowDialog() == true)
            {
                ...
                {
                    ResultsGrid.ItemsSource = _watchRepo.GetMechanicalWatchesCheaperThan(price);
                }
        ...
        private void QueryManufacturersByValue_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (dialog.ShowDialog() == true)
            {
                if (decimal.TryParse(dialog.ResponseText, out decimal value))
                {
                    ResultsGrid.ItemsSource = _manufacturerRepo.GetManufacturersByTotalValue(value);
                }
                else { MessageBox.Show("Некорректный ввод. Введите число."); }
            }
        }
        ...
        private void AddStock_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (addWindow.ShowDialog() == true)
            {
                try
                {
                    _stockRepo.Add(addWindow.CurrentStock);
                    LoadData();
                }
                catch (Exception ex) { MessageBox.Show($"Ошибка добавления: {ex.Message}"); }
            }
        ...
        {
            var selected = StockGrid.SelectedItem as Stock;
            if (selected == null) { MessageBox.Show("Выберите запись для редактирования."); return; }
            var editWindow = new AddEditStockWindow(selected);
            if (editWindow.ShowDialog() == true)
            {
                try
                {
                    _stockRepo.Update(editWindow.CurrentStock);
                    LoadData();
                }
                catch (Exception ex) { MessageBox.Show($"Ошибка изменения: {ex.Message}"); }
            }
